...
public class ResourceController implements BeanFactoryAware {
    ...
    public List<ResourceDTO> getAllResources() throws ContentValidationException, ResourceAccessError {
        log.trace(Constants.GET_ALL_RESOURCES);
        List<ResourceDTO> resources = resourceService.getPersistedResources();
        log.trace(resources.toString());
        return resources;
    }

    @RequestMapping(value = "/v" + Constants.REST_API_1_0 + "/" + Constants.GET_RELEVANT_REVISIONS + "/{resourceId}", method = RequestMethod.GET)

    public List<RevisionDTO> getResourceRevisions(@PathVariable Integer resourceId) throws ContentValidationException, ResourceAccessError {
        log.trace(Constants.GET_RELEVANT_REVISIONS);
        List<RevisionDTO> resourceRevisions = resourceService.getResourceRevisions(resourceId);
        log.trace(resourceRevisions.toString());

...
import com.sastix.cms.common.api.EmailAPI;
...
@Service
@Slf4j
public class CmsClient implements ContentClient, LockClient, CacheClient, BeanFactoryAware, EmailAPI {
    ...
    public List<ResourceDTO> getPersistedResources() {
        String url = apiVersionClient.getApiUrl() + "/" + Constants.GET_ALL_RESOURCES;
        log.trace("API call: " + url);
        List<ResourceDTO> resources = retryRestTemplate.getForObject(url, List.class);
        log.trace("Response:" + resources.toString());
        return resources;
    }

    @Override
    public void sendEmailWithResourceAttachment(SendEmailDTO sendEmailDTO) {
        String url = apiVersionClient.getApiUrl() + "/" + Constants.SEND_EMAIL_ATTACHMENT;
        log.trace("API call: " + url);
        retryRestTemplate.postForObject(url, sendEmailDTO, SendEmailDTO.class);
    }

    @Override
    public List<RevisionDTO> getResourceRevisions(Integer resourceId) {
        String url = apiVersionClient.getApiUrl() + "/" + Constants.GET_RELEVANT_REVISIONS + "/" + resourceId;
        log.trace("API call: " + url);
        List<RevisionDTO> revisions = retryRestTemplate.getForObject(url, List.class);
        log.trace("Response:" + revisions.toString());
        return revisions;
    }
